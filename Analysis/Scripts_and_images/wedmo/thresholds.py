...
for j in range(1000):
	...
	while i< taille + 1 and sortie:
		...
		YpredictionConsecutiveSignal = probabilityPredictionMainSub(clf4, clf3_1, clf3_2, clf3_3, clf3_4, XconsecutiveSignal, XconsecutiveSignal1, XconsecutiveSignal2, XconsecutiveSignal3, XconsecutiveSignal4, XuntouchedF, XuntouchedF1, XuntouchedF2, XuntouchedF3, XuntouchedF4, threshold)
	 	...
	 	falsePositive = 0
	 	k = 0
	 	while k < len(YpredictionConsecutiveSignal) and sortie:
			...
			for l in range(len(YpredictionConsecutiveSignal[k])):
				if YpredictionConsecutiveSignal[k][l] == 1 and placesToIndex(YuntouchedF[k]) != l:
					falsePositive += 1
			k+=1
		print("Falseneg "+str(falseNegative))
		print("Falsepos "+str(falsePositive))
		if falseNegative > falsePositive:
			thresholds.append(threshold+i*0.5*maximum/float(taille*100))
			sortie = False
			print("done")	
...
print("threshold Main Sub")
print(np.mean(thresholds))
print(np.std(thresholds))
print(mean_confidence_interval(thresholds))
print(mean_confidence_interval(thresholds, 0.99))
print(mean_confidence_interval(thresholds, 0.999))
print(mean_confidence_interval(thresholds, 0.9999))
