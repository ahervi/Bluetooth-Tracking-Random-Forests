...
def probabilityPredictionSub(clf3_1, clf3_2, clf3_3, clf3_4, X1, X2, X3, X4,XuntouchedF1,XuntouchedF2,XuntouchedF3,XuntouchedF4, threshold):
	YpredictionConsecutiveSignal = []
	...
	for i in range(len(probas3_1)):
		...
		probas = [float(probas3_1[i][j]+probas3_2[i][j]+probas3_3[i][j]+probas3_4[i][j])/4 for j in range(len(probas3_1[i]))]
		maxIndex = max(enumerate(probas),key=lambda x: x[1])[0]
		toAppend = [0]*len(probas)
...
def probabilityPredictionMainSub(clf4, clf3_1, clf3_2, clf3_3, clf3_4, X, X1, X2, X3, X4, XuntouchedF,XuntouchedF1,XuntouchedF2,XuntouchedF3,XuntouchedF4, threshold):
	YpredictionConsecutiveSignal = []
	...
	for i in range(len(probas3_1)):
		...
		probas = [float(probas4[i][j] + probas3_1[i][j]+probas3_2[i][j]+probas3_3[i][j]+probas3_4[i][j])/5 for j in range(len(probas3_1[i]))]
		maxIndex = max(enumerate(probas),key=lambda x: x[1])[0]
		toAppend = [0]*len(probas)
...
for j in range(1000):
	...
	clf3_1, YpredictionConsecutiveSignal3_1 = givesClassifier("3_1", DBS, XconsecutiveSignalCombinaisons1, XconsecutiveSignal1, YconsecutiveSignalCombinaisons, YconsecutiveSignal, XuntouchedF1, YuntouchedF)
	clf3_2, YpredictionConsecutiveSignal3_2 = givesClassifier("3_2", DBS, XconsecutiveSignalCombinaisons2, XconsecutiveSignal2, YconsecutiveSignalCombinaisons, YconsecutiveSignal, XuntouchedF2, YuntouchedF)
	clf3_3, YpredictionConsecutiveSignal3_3 = givesClassifier("3_3", DBS, XconsecutiveSignalCombinaisons3, XconsecutiveSignal3, YconsecutiveSignalCombinaisons, YconsecutiveSignal, XuntouchedF3, YuntouchedF)
	clf3_4, YpredictionConsecutiveSignal3_4 = givesClassifier("3_4", DBS, XconsecutiveSignalCombinaisons4, XconsecutiveSignal4, YconsecutiveSignalCombinaisons, YconsecutiveSignal, XuntouchedF4, YuntouchedF)
	...
	YpredictionConsecutiveSignal = probabilityPredictionSub(clf3_1, clf3_2, clf3_3, clf3_4, XconsecutiveSignal1, XconsecutiveSignal2, XconsecutiveSignal3, XconsecutiveSignal4, XuntouchedF1, XuntouchedF2, XuntouchedF3, XuntouchedF4, threshold)
	...
	falsePositive =100 * float(falsePositive) / (len(YpredictionConsecutiveSignal)*(len(YpredictionConsecutiveSignal[0])-1))
	falseNegative = 100*float(falseNegative) / len(YpredictionConsecutiveSignal)
	#graph3.append(100-100*float(errors)/len(YpredictionConsecutiveSignal))
	graphP.append(falsePositive)
	graphN.append(falseNegative)
	graphPN.append(falsePositiveNumber)
	graphNN.append(falseNegativeNumber)
	#print(100*(float(i)/maximum))
	#print(100*float(errors)/len(YpredictionConsecutiveSignal))
print("Sub")
print("Equal")
print("false positive")
print(np.mean(graphP))
print(np.std(graphP))
print(mean_confidence_interval(graphP))
print(mean_confidence_interval(graphP, 0.99))
print(mean_confidence_interval(graphP, 0.999))
print(mean_confidence_interval(graphP, 0.9999))

print("false negative")

print(np.mean(graphN))
print(np.std(graphN))
print(mean_confidence_interval(graphN))
print(mean_confidence_interval(graphN, 0.99))
print(mean_confidence_interval(graphN, 0.999))
print(mean_confidence_interval(graphN, 0.9999))
