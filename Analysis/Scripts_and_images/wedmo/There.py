...
def probabilityPredictionMain(clf4, X, XuntouchedF, threshold):
	...
	for i in range(len(probas4)):
		...
		toAppend = []
		if probas[maxIndex] >= threshold:
			YpredictionConsecutiveSignal.append([probas[maxIndex], places[maxIndex]])
		else:
			YpredictionConsecutiveSignal.append([probas[maxIndex],'unknown_location'])
...
for i in range(0,taille + 1):
	...
	for k in range(len(YpredictionConsecutiveSignal)):
		...
		if YpredictionConsecutiveSignal[k][1] != YuntouchedF[k]:
			# remplacer par un 1 ?
			
			falseNegative += 1
			#ratio += 1
	falseNegative = 100 * float(falseNegative) / len(YpredictionConsecutiveSignal)
	graph3.append(falseNegative)
...
plt.legend(['Main model (probabilistic)'], loc='upper left')
plt.ylabel('% of errors on prediction sample')
plt.xlabel('acceptance threshold')
plt.savefig("there1.png")
