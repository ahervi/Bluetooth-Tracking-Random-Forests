...
def timeseparation(XconsecutiveSignalCombinaisons, minutes):
	...
	for v in range(len(XconsecutiveSignalCombinaisons)):
		...
		if XconsecutiveSignalCombinaisons[v][1] - oldTime > minutes:
...
for minutes in range(1, 10*60):
	...
	if len(Xpartial) > 2:
		for j in range(len(Xpartial) - 2):
			clfRepeat = 100
			for m in range(clfRepeat):
				print(str(j) + " out of " + str(len(Xpartial) - 2))
				sizeTest = 1
			...
			meanLocale = meanLocale / float(clfRepeat)
		if len(Xpartial) - 2 != 0:
			print(len(per))
			per.append(minutes/60 + (minutes%60)*(float(100)/60)*0.01)
