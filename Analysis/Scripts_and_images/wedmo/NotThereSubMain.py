...
graphP = [sum(x)/float(10) for x in zip(*graphP)]
per = per[:taille+1]
print(len(graphP))
print(len(per))
graphN = [sum(x)/float(10) for x in zip(*graphN)]
graphPN = [sum(x)/float(10) for x in zip(*graphPN)]
graphNN = [sum(x)/float(10) for x in zip(*graphNN)]
...
axes2.set_ylabel('false discovery rate', color = 'green')
