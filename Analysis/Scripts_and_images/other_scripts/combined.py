...
axes = plt.gca()
axes.set_ylim([0,45])
plt.plot(per,graph, next(linecycler))
#plt.plot(per,graph1, next(linecycler))
#plt.plot(per,graph2, next(linecycler))
plt.plot(per,graph3, next(linecycler))
#plt.plot(per,graph4, next(linecycler))
...
plt.legend(['Main type', 'Main and Submodels type (probabilistic)'], loc='upper right')
plt.ylabel('% of errors on prediction sample')
plt.xlabel('% of samples used for training')
plt.savefig("models100.png")
