...
#Do not use matplotlib good for testing reception from all RPIS
...
def probabilityPredictionSub(sizeTest, clf3_1, clf3_2, clf3_3, clf3_4, X1, X2, X3, X4):
    ...
    probas3_1 = clf3_1.predict_proba([X1, X1])
    probas3_2 = clf3_2.predict_proba([X2, X2])
    probas3_3 = clf3_3.predict_proba([X3, X3])
    probas3_4 = clf3_4.predict_proba([X4, X4])
...
def on_message(client, userdata, msg):
    ...
    positiv = RSSI1[1:]
    if positiv.isdigit() and - int(positiv) < 0:
        if BSSID1 == "C7:64:6C:03:B9:40":
            ...
            for l in range(MAXID):
                ...
                if (NEWTIME[ID1 - 1] - OLDTIME[l - 1]) > TIME_WINDOW and l != ID1 - 1:  
            ...
            if not 0 in RSSIS:
                ...
                YpredictionSignal = probabilityPredictionSub(0, randomForestModels[0], randomForestModels[1], randomForestModels[2], randomForestModels[3], RSSIS, triList[0][0], triList[1][0], triList[2][0])
